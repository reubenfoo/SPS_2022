...
# data.partition.ids <- read.table('data-partition-ids-subtype-nonormal.txt')
...
for (i in 1:200){
  ...
  all.fit <- train(x=dataset.train[,1:(ncol(dataset.train)-2),drop=F],y=dataset.train$labels,method='ranger',tuneGrid=tunegrid)
  boruta.fit <- train(x=dataset.train[,selected.features.ids,drop=F], y=dataset.train$labels,method='ranger',tuneLength=5)
  sps.fit <- train(x=dataset.train[,sps.ids,drop=F],y=dataset.train$labels,method='ranger',tuneLength=5)
  pam.fit <- train(x=dataset.train[,pam.ids,drop=F],y=dataset.train$labels,method='ranger',tuneLength=5)
  ...
  predictions4 <- predict(pam.fit, dataset.test[,pam.ids,drop=F])

  ## Get prediction accuracy
  accuracy1 <- sum(predictions1 == dataset.test$labels)/length(predictions1)
  accuracy2 <- sum(predictions2 == dataset.test$labels)/length(predictions2)
  accuracy3 <- sum(predictions3 == dataset.test$labels)/length(predictions3)
  accuracy4 <- sum(predictions4 == dataset.test$labels)/length(predictions4)
  
  # write.table(t(c('S/N','All','Boruta','SPS','PAM')), file='boruta-normrun-everything.txt', append=T, col.names=F, row.names=F)
  write.table(t(c(i,accuracy1, accuracy2, accuracy3, accuracy4)), file='boruta-normrun-everything.txt', append=T, col.names=F, row.names=F)
  # write.table(t(c(paste('Resample',i),selected.features.ids)), file='subtype-boruta-impforest-80train-100runs-normfeatures.txt', append=T, col.names=F, row.names=F)

  # write.table(t(c(paste('Resample',i),confirmed.features.ids)), file='subtype-boruta-impforest-80train-100runs-normfeatures-confirmed.txt', append=T, col.names=F, row.names=F)
  # write.table(t(c(paste('Resample',i),tentative.features.ids)), file='subtype-boruta-impforest-80train-100runs-normfeatures-tentative.txt', append=T, col.names=F, row.names=F)
}
...
acc.list <- read.table('subtype-boruta-normrun.txt', header=T)[,-1,drop=F]
...
wilcox.test(x = acc.list$SPS, y = acc.list$PAM)
# print(mean(acc.list$PAM))
...
p2 <- ggplot(melt(acc.list), aes(x=variable, y=value, color=variable)) + 
  ...
  geom_violin(width=1, alpha=0.4, aes(fill=variable)) +
  geom_boxplot(fatten=4, alpha=0.5) +
  xlab('Model') + ylab('Test Accuracy') +
...
# sps.acc.list <- read.table('boruta-80train-sps-only.txt')


## plot frequency of occurence of genes in Boruta features
